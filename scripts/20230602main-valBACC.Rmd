---
title: "Observe-ACS | June, 2023"
subtitle: "Validation on BACC Cohort / mixed train/test"
author: | 
...
knit: (function(input, ...) {
    rmarkdown::render(
      input,
      output_file = paste0(  
        Sys.Date(), '-',  "observeACS_analysis_k10_validation_mixed", '.html'
      ),
...
filename_plot_tune_race_ranking <- paste0("output/plots/tune_grid/", Sys.Date(), "_tune_race_ranking_v_", v,"_", mixed_cohort_naming,".svg")
...
if( topmodel1_race != "KNN" &  topmodel1_race != "full_quad_KNN" ) {
  ...
  filename_plot_tune_race_hyperpars_topmodel1 <- paste0("output/plots/tune_grid/", Sys.Date(), "_tune_race_hyperpars_topmodel1_v_", v,"_", mixed_cohort_naming,".svg")
...
if( topmodel2_race != "KNN" &  topmodel2_race != "full_quad_KNN" ) {
  ...
  filename_plot_tune_race_hyperpars_topmodel2 <- paste0("output/plots/tune_grid/", Sys.Date(), "_tune_race_hyperpars_topmodel2_v_", v,"_", mixed_cohort_naming,".svg")
...
for (models in 1:nrow(race_results) ) {
  ...
  if(save.FILE==TRUE) {
    ...
    testresults_save <- paste0(format(Sys.Date(), "%Y%m%d"),  "_TEST_RESULTS_", wflow_id, "_v_", v,"_", mixed_cohort_naming)
    ...
    modelname <- paste0(format(Sys.Date(), "%Y%m%d"),  "_MODEL_", wflow_id, "_v_", v,"_", mixed_cohort_naming)
    ...
    saveRDS(vetiver, file = filename.model.vetiver)
  ...
  if(save.FILE == TRUE) {
    filename_plot_ROC_wflow_id <- paste0("./output/plots/ROC/", Sys.Date(), "_ROC_", wflow_id, "_v_", v,"_", mixed_cohort_naming, ".svg")
  ...
  if(save.FILE == TRUE) {
    filename_plot_calibration_wflow_id <- paste0("./output/plots/calibration_plots/", Sys.Date(), "_calibration_", wflow_id,
                                                  "_v_", v,"_", mixed_cohort_naming, ".svg")
...
if(save.FILE==TRUE) {
  filename.df.xlsx <- paste0("./output/performance_summary_df/", format(Sys.Date(), "%Y%m%d"),
                             "_performance_summary_table_original_cutoff_v_", v,"_", mixed_cohort_naming,".xlsx")
  filename.df.rds <- paste0("./output/performance_summary_df/", format(Sys.Date(), "%Y%m%d"), 
                           "_performance_summary_table_original_cutoff_v_", v,"_", mixed_cohort_naming,".rds")
