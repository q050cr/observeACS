---
title: "Cluster Analysis in Observe-ACS | March, 2023"
subtitle: "Subset of Clustering Variables"  # useSubsetClusterVars = TRUE
author: | 
...
if(useSubsetClusterVars == TRUE) {
  x <- scale(impdat.merged %>% 
               mutate(V_h_lvdys_grad = as.numeric(V_h_lvdys_grad)) %>% 
               select(V_age, V_sex_f1_m0, V_grace_score, 
                      # risk, abnormal LV?
                      V_h_diabetes, V_h_lvdys_grad, V_ekg_sinus_normal, 
                      # history CAD/MI/PCI/CABG ANY
                      V_h_khk, # hier enthalten, geprüft
                      # CKD?
                      V_t0_krea_value, V_t0_hst_value,
                      # trop
                      V_t0_hstnt_value, V_delta_c_first_tnt_relativ, 
                      # inflammation
                      V_t0_crp_value, V_vit_temperatur,
                      # Hb
                      V_t0_hb_value)
  )
} else {
...
```{r}
# did not use smaller subset of variables here... 
